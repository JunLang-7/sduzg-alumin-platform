...
`GET /api/v1/alumni/me` 返回当前校友账号绑定的校友详情，响应字段与 `GET /api/v1/alumni/{id}` 一致。

`PUT /api/v1/alumni/me` 仅允许校友修改以下四个字段：
...
响应返回更新后的本人资料。即使请求体包含其他字段，服务端也不会写入上述四个字段以外的校友档案字段；如允许校友修改更多字段，应在需求确认后调整接口校验规则。
