...
export function AuditOperationHistoryCard({ alumniId }: Props) {
  ...
  const loadHistory = useCallback(async () => {
    setLoading(true);
    try {
      const query: AuditQuery = { page: 1, page_size: 100, target_id: alumniId };
  ...
  return (
    <>
      <Card
        ...
        title={<span><HistoryOutlined /> 操作历史 <Typography.Text type="secondary">共 {total} 条</Typography.Text></span>}
      >
