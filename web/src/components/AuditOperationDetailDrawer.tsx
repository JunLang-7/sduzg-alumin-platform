      ...
      {record ? (
        <div className="audit-drawer">
          <div className="audit-drawer-heading">
            <div>
              <Typography.Text type="secondary">{record.target_type === 'alumni_batch' ? '校友档案批量操作' : '校友档案操作'}</Typography.Text>
              <h2>{record.operator}{actionLabel(record.action)}了 {record.target_name}</h2>
              <p>{record.created_at}</p>
            </div>
          </div>

          <Descriptions size="small" column={2} bordered>
            <Descriptions.Item label="操作人">{record.operator} · {record.operator_role_label || '操作人'}</Descriptions.Item>
            <Descriptions.Item label="操作类型"><Tag color={actionColor(record.action)}>{actionLabel(record.action)}</Tag></Descriptions.Item>
            <Descriptions.Item label="操作来源">{sourceLabel(record.source)}</Descriptions.Item>
            <Descriptions.Item label="管理范围">{record.management_scope || '全部校友'}</Descriptions.Item>
            ...
            <Descriptions.Item label="记录状态" span={record.reason ? 1 : 2}><Tag color={statusColor(record.status)}>{statusLabel(record.status)}</Tag></Descriptions.Item>
            {record.reason ? <Descriptions.Item label="操作说明">{record.reason}</Descriptions.Item> : null}
          </Descriptions>

          <Divider orientation="left">字段变化</Divider>
          {record.changes && record.changes.length > 0 ? (
            record.changes.map((change) => (
              <Card size="small" key={`${change.field_name}-${change.field_label}`} className="audit-drawer-diff">
                <div className="audit-drawer-field-head"><strong>{change.field_label}</strong></div>
                <div className="audit-drawer-values">
                  <Typography.Text delete type="secondary">{change.old_value || '未填写'}</Typography.Text>
                  <ArrowRightOutlined />
        ...
        <Space className="audit-drawer-readonly"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="未选择操作记录" /></Space>
      )}
    </Drawer>
  );
}
