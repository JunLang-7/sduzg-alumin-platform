...
export function AdminUsersPage() {
  ...
  useEffect(() => {
    adminsApi
      .listDataDomains()
      .then(setDomains)
      .catch((error: Error) => message.error(error.message || '可管理范围加载失败'));
  }, []);
  ...
  const openAccessModal = async (record: AdminUser) => {
    ...
    accessForm.setFieldsValue({
      domain_ids: record.domains.map((domain) => domain.id),
      permissions: record.permissions,
    });
    try {
      const detail = await adminsApi.detail(record.id);
      accessForm.setFieldsValue({
        domain_ids: detail.domains.map((domain) => domain.id),
        permissions: detail.permissions,
      });
    } catch (error) {
      message.error((error as Error).message || '管理员授权加载失败');
    }
  };
  ...
  const saveAccess = async () => {
    ...
    try {
      ...
      message.error((error as Error).message || '保存授权失败');
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns: ColumnsType<AdminUser> = [
    ...
    { title: '姓名', dataIndex: 'real_name' },
    {
      title: '可管理范围',
      dataIndex: 'domains',
      render: (value: DataDomain[]) =>
        value.map((domain) => <Tag key={domain.id}>{domain.name}</Tag>),
    },
    {
      title: '敏感数据',
      dataIndex: 'permissions',
      render: (value: string[]) =>
        value.includes(permissionCodes.sensitiveRead) ? (
          <Tag color="green">可查看</Tag>
        ) : (
          <Tag>不可查看</Tag>
        ),
    },
    {
      ...
      dataIndex: 'permissions',
      render: (value: string[]) =>
        value.includes(permissionCodes.filesManage) ? (
          <Tag color="blue">可管理</Tag>
        ) : (
          <Tag>不可管理</Tag>
        ),
    },
        ...
        record.role === 'super_admin' ? (
          ...
          <Space size={4}>
            <Button
              type="link"
              icon={<EditOutlined />}
              onClick={() => void openAccessModal(record)}
            >
              编辑授权
            </Button>
            <Popconfirm
  ...
  return (
    <>
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: '请输入账号' },
              {
                ...
                message: '账号须为 4-32 位，以字母开头，仅可包含字母、数字、下划线和连字符',
              },
            ]}
          >
      ...
      <Modal
        title={`编辑授权${editing ? ` · ${editing.account}` : ''}`}
  ...
  return (
    <>
      <Form.Item
        label="可管理范围"
        name="domain_ids"
        rules={[{ required: true, message: '请至少选择一项可管理范围' }]}
      >
        <Select
          mode="multiple"
          placeholder="选择可管理的培养类别"
          options={domains.map((domain) => ({ label: domain.name, value: domain.id }))}
        />
      </Form.Item>
      <Form.Item label="功能权限" name="permissions">
