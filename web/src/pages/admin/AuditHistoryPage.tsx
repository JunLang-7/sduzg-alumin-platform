...
export function AuditHistoryPage() {
  ...
  const [data, setData] = useState<{ items: AuditOperation[]; total: number }>({
    items: [],
    total: 0,
  });
  ...
  const requestQuery = useMemo<AuditQuery>(
    () =>
      buildAuditQuery({
        ...
        managementScope: scopeFilter || undefined,
        action: actionFilter,
      }),
    ...
    () => [
      {
        ...
        width: 168,
        render: (value: string) => <Typography.Text>{formatAuditDateTime(value)}</Typography.Text>,
      },
      {
        title: '操作人',
        dataIndex: 'operator',
        ...
        render: (_value, record) => (
          <span className="audit-table-target">
            {record.target_id && record.target_type === 'alumni_profile' ? (
              <Link className="audit-table-alumni-link" to={`/alumni/${record.target_id}`}>
                {record.target_name}
              </Link>
            ) : (
  ...
  return (
    <div className="audit-page audit-history-page">
      <PageHeader title="操作历史" description="查看管理员和校友本人对校友档案的操作记录。" />
      ...
      <Card
        ...
        title={
          <Space>
            <AuditOutlined />
            操作历史 <Typography.Text type="secondary">共 {data.total} 条</Typography.Text>
          </Space>
        }
      >
